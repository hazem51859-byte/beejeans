...
const customers = [
  ...
  { name: 'هاني لافلي', balance: -165, phone: null }, // هو ليه عندنا (علينا)
  ...
  { name: 'ام احمد قنا', balance: -240, phone: null }, // هو ليه عندنا (علينا)
  ...
  { name: 'ناصر مرعي جراند مول', balance: -240, phone: null }, // هو ليه عندنا (علينا)
  { name: 'رغده اكتوبر', balance: 3600, phone: null },
  { name: 'احمد مدحت الفيوم', balance: -515, phone: null }, // هو ليه عندنا (علينا)
  ...
  { name: 'مينا صبري ملاوي', balance: -840, phone: null }, // هو ليه عندنا (علينا)
  { name: 'اسامه عبده بنها الكبري', balance: 1060, phone: null },
  { name: 'ابو شروق المنوفيه', balance: -400, phone: null }, // هو ليه عندنا (علينا)
  ...
  { name: 'ام يوسف طنطا', balance: -2860, phone: null } // هو ليه عندنا (علينا)
];

async function fixCustomersBalances() {
  try {
    ...
    let addedCount = 0;
    let invoicesWeOwe = 0; // احنا لينا عنده
    let invoicesHeOwes = 0; // هو ليه عندنا
    ...
    for (const customerData of customers) {
      ...
      // إذا كان الرقم سالب: هو ليه عندنا (علينا فلوس)
      else if (customerData.balance < 0) {
        const amountOwed = Math.abs(customerData.balance);
        
        // إنشاء فاتورة بالمبلغ السالب (كأنه احنا اشترينا منه)
        const invoiceNumber = `OPEN-CREDIT-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
        ...
        await prisma.officeInvoice.create({
          data: {
            ...
            discountAmount: 0,
            total: amountOwed,
            totalCost: amountOwed * 0.6,
            profit: amountOwed * 0.4,
            paymentMethod: 'DEFERRED',
            paidAmount: amountOwed, // مدفوعة كاملة
            remainingAmount: 0,
            status: 'PAID',
            notes: 'رصيد افتتاحي - هو ليه فلوس عندنا (علينا) - 1/9/2026',
            createdBy: adminUser.id,
            createdAt: openingDate,
            updatedAt: openingDate
          }
        ...
        // إضافة الدفعة اللي احنا دفعناها (بالزيادة)
        await prisma.officeInvoicePayment.create({
          data: {
            invoiceId: (await prisma.officeInvoice.findFirst({
              where: { invoiceNumber },
              select: { id: true }
            })).id,
            amount: amountOwed,
            paymentMethod: 'CASH',
            notes: 'رصيد افتتاحي - مدفوع كامل - 1/9/2026',
            createdBy: adminUser.id,
            paymentDate: openingDate
          }
        });
        
        console.log(`   💸 هو ليه عندنا (علينا): ${amountOwed.toLocaleString()} ج.م`);
        invoicesHeOwes++;
      }
    }
    ...
    console.log(`   - فواتير (هم ليهم عندنا - علينا): ${invoicesHeOwes}`);
