...
async function fixCustomersNotes() {
  try {
    console.log('🚀 بدء تصحيح أرصدة العملاء مع Notes واضحة...\n');
      ...
      if (customerData.balance > 0) {
        ...
        const customer = await prisma.customer.create({
          data: {
            ...
            phone: phoneToUse,
            address: '',
            notes: '✅ رصيد افتتاحي 1/9/2026 - لينا عنده (ديون)',
            isActive: true,
            walletBalance: 0
          }
        });
        ...
        await prisma.officeInvoice.create({
          data: {
            ...
            status: 'PENDING',
            notes: '✅ رصيد افتتاحي 1/9/2026 - احنا لينا عنده',
        ...
        console.log(`✅ ${customerData.name}: لينا عنده ${customerData.balance.toLocaleString()} ج.م`);
        invoicesWeOwe++;
        addedCount++;
      ...
      // إذا كان الرقم سالب: هو ليه عندنا (علينا فلوس)
        ...
        const customer = await prisma.customer.create({
          data: {
            ...
            phone: phoneToUse,
            address: '',
            notes: '⚠️ رصيد افتتاحي 1/9/2026 - علينا ليه فلوس',
            isActive: true,
            walletBalance: -amountOwed // رصيد سالب = احنا مدينين له
          }
        });
        
        console.log(`⚠️  ${customerData.name}: علينا ليه ${amountOwed.toLocaleString()} ج.م`);
        creditsHeOwes++;
        addedCount++;
      }
    }
    ...
    console.log('\n✅ اكتمل!');
    console.log(`\n📊 الإحصائيات:`);
    console.log(`   - إجمالي العملاء: ${addedCount}`);
    console.log(`   - ✅ احنا لينا عندهم: ${invoicesWeOwe} عميل`);
    console.log(`   - ⚠️  علينا ليهم: ${creditsHeOwes} عميل`);
    ...
    console.log(`\n💰 الملخص المالي:`);
    console.log(`   - ✅ احنا لينا عندهم: ${totalWeOwe.toLocaleString()} ج.م`);
    console.log(`   - ⚠️  علينا ليهم: ${totalHeOwes.toLocaleString()} ج.م`);
    console.log(`   - 📊 الصافي: ${netBalance.toLocaleString()} ج.م`);
    
    console.log(`\n⚠️  العملاء اللي علينا ليهم فلوس:`);
    customers.filter(c => c.balance < 0).forEach(c => {
      console.log(`   - ${c.name}: ${Math.abs(c.balance).toLocaleString()} ج.م`);
    });
