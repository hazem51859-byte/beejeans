...
// البحث عن آخر section قبل نهاية الـ return
// هنضيف الجرود بعد جدول فواتير المكتب

const auditsSection = `
          {/* ========== قسم الجرود والخسائر ========== */}
          {auditsData?.data && auditsData.data.totalAudits > 0 && (
            <div className="bg-white rounded-lg shadow-md p-6 mb-6 page-break-before">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-500">
                  ...
                  <div className="text-2xl font-bold text-blue-800">{auditsData.data.totalAudits}</div>
                </div>

                <div className="bg-orange-50 p-4 rounded-lg border-l-4 border-orange-500">
                  <div className="text-sm text-orange-600 mb-1">إجمالي كمية العجز</div>
                  <div className="text-2xl font-bold text-orange-800">{auditsData.data.totalShortageQty} قطعة</div>
                </div>

                <div className="bg-red-50 p-4 rounded-lg border-l-4 border-red-500">
                  <div className="text-sm text-red-600 mb-1">خسائر العجز (بالتكلفة)</div>
                  <div className="text-2xl font-bold text-red-800">{auditsData.data.totalShortageValue.toFixed(2)} ج.م</div>
                  <div className="text-xs text-red-600 mt-1">💰 الخسارة الفعلية</div>
                </div>

                <div className="bg-emerald-50 p-4 rounded-lg border-l-4 border-emerald-500">
                  <div className="text-sm text-emerald-600 mb-1">صافي الخسارة</div>
                  <div className="text-2xl font-bold text-emerald-800">
                    {auditsData.data.netLoss >= 0 
                      ? \`\${auditsData.data.netLoss.toFixed(2)} ج.م\`
                      : \`+\${Math.abs(auditsData.data.netLoss).toFixed(2)} ج.م\`
                    }
                  </div>
                  <div className="text-xs text-emerald-600 mt-1">
                    {auditsData.data.netLoss >= 0 ? '📉 خسارة' : '📈 مكسب'}
                  </div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full bg-white border border-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {auditsData.data.audits.map((audit) => (
                      <tr key={audit.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-right text-red-700 font-bold">
                          {audit.actualLoss.toFixed(2)} ج.م
                        </td>
                        <td className="px-4 py-3 text-sm text-right font-bold">
                          <span className={audit.netLoss >= 0 ? 'text-red-700' : 'text-emerald-700'}>
                            {audit.netLoss >= 0 
                              ? \`-\${audit.netLoss.toFixed(2)}\`
                              : \`+\${Math.abs(audit.netLoss).toFixed(2)}\`
                            } ج.م
                          </span>
                        </td>
                      </tr>
                  ...
                  <tfoot className="bg-gray-50">
                    <tr className="font-bold">
                      ...
                      <td className="px-4 py-3 text-center text-orange-800">{auditsData.data.totalShortageQty}</td>
                      <td className="px-4 py-3 text-right text-red-700">{auditsData.data.totalShortageValue.toFixed(2)} ج.م</td>
                      <td className="px-4 py-3 text-right">
                        <span className={auditsData.data.netLoss >= 0 ? 'text-red-700' : 'text-emerald-700'}>
                          {auditsData.data.netLoss >= 0 
                            ? \`-\${auditsData.data.netLoss.toFixed(2)}\`
                            : \`+\${Math.abs(auditsData.data.netLoss).toFixed(2)}\`
                          } ج.م
                        </span>
                      </td>
                    </tr>
              ...
              {auditsData.data.audits.some(a => a.items && a.items.length > 0) && (
                <div className="mt-6">
                            ...
                            <div key={idx} className="flex items-center justify-between p-2 bg-white rounded border border-red-100">
                              ...
                              <div className="text-right">
                                <div className="text-sm font-bold text-red-700">
                                  {item.actualDifferenceValue.toFixed(2)} ج.م
                                </div>
...
// البحث عن مكان مناسب للإضافة - بعد قسم فواتير المكتب
const insertAfter = '</div>\n        </div>\n      </div>\n    );\n  }\n}';

if (content.includes(insertAfter)) {
  console.log('❌ Could not find insertion point');
  console.log('Will append before the closing return statement');
  
  // نبحث عن آخر return في الملف
  const returnIndex = content.lastIndexOf('return (');
  if (returnIndex !== -1) {
    // نبحث عن آخر closing div قبل نهاية الـ return
    const lastDivIndex = content.lastIndexOf('</div>\n        </div>\n      </div>\n    );');
    
    if (lastDivIndex !== -1) {
      // نضيف الـ audits section قبل آخر closing divs
      content = content.slice(0, lastDivIndex) + auditsSection + '\n' + content.slice(lastDivIndex);
      
      fs.writeFileSync(filePath, content, 'utf8');
      console.log('✅ Added audits section to MonthlyReport.jsx');
    } else {
  ...
  console.log('❌ Insertion point not found, will try alternative method');
}
