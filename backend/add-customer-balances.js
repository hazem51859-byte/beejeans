...
async function addCustomerBalances() {
  console.log('💰 بدء إضافة الأرصدة للعملاء...\n');
  ...
  try {
    ...
    // الحصول على أي مستخدم أدمن لربط الفواتير به
    ...
    await prisma.$transaction(async (tx) => {
      for (const customerData of customerBalances) {
        ...
        // لو الرصيد صفر، نتجاهل
        if (balance === 0) {
          continue;
        }

        if (balance > 0) {
          // العميل مدين لينا - نعمل فاتورة مكتب
          const invoiceNumber = `INIT-${Date.now()}-${customer.id.substring(0, 8)}`;
          ...
          await tx.officeInvoice.create({
            data: {
              ...
              discountAmount: 0,
              total: balance,
              totalCost: balance * 0.7, // تقدير تكلفة 70% من سعر البيع
              profit: balance * 0.3,
          ...
          // العميل ليه فلوس عندنا (رصيد سالب) - نحط في المحفظة
    ...
    console.log(`   📈 عملاء مدينين (فواتير): ${positiveBalances}`);
